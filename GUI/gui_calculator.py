...
class MyPyCalculator(CalcWidget):
    ...
    def config_keyboard(self) -> None:
        ...
        :return: None
        """
        btn = [["7", "8", "9", "C", "Del"], ["4", "5", "6", "+", "/"], ["1", "2", "3", "-", "*"]]
        for pos in range(5):
            self.keyboard.grid.addWidget(CalcButton(self.keyboard, f'{btn[0][pos]}'), 1, pos, 1, 1)
            self.keyboard.grid.addWidget(CalcButton(self.keyboard, f'{btn[1][pos]}'), 2, pos, 1, 1)
